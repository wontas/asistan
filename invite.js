...
client.on("guildMemberAdd", (member) => {
    ...
    guild.fetchInvites().then(invites => {
        ...
        if(channel){
       channel.send(`> <a:hg:856471278689320961> **${member} Adlı Kişi Sunucuya Katıldı. Davet Eden Kişi ${invite.inviter.tag} Toplam ${total + bonus} Davete Ulaştı. Tebrikler!** `)
        }
    }).catch();
});
    ...
    db.add(`invites.${data.inviter}.leave`, 1);
     if(channel){
        let user = client.users.cache.get(data.inviter)
     	channel.send(`> <a:bb:856471316898906153> **${member.user.tag} Sunucudan Ayrıldı. Davet Eden Kişi ${user.tag} Toplam Daveti ${Number(total) + Number(bonus)}** `)
     }
});
