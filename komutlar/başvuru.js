...
exports.run = async (client, message, args) => { 
	...
	if(basvurdata) return message.reply(`<a:carpi:839244319197364316> **Başvurular Geçici Olarak Durdurulmuştur.**`);
	
	const bandata = await db.get(`ban.${message.author.id}`)
	if(bandata) return message.reply("<a:carpi:839244319197364316> **Başvurulardan Banlısın**");
  ...
  const sorular = [
    '**İsmini Ve Yaşını Öğrenebilirmiyim?** İsim/Yaş',
    '**Günde Kaç Saat Aktifsiniz?** 1/24 Saat',
    '**Daha Önce Başka Büyük Sunucuda Yetkili Oldunuz Mu? Olduysanız Hangi Sunucu ?** Evet/Hayır Sunucu Adı',
    '**Discord Platformunda Kaç Senedir Bulunuyorsunuz?** Yıl',
    '**Üyelere Hangi Konuda Destek Vermeyi Planlıyorunuz?** Discord-Destek / Kod-Destek / Abone Rolü Verme',
    '**Neden Anka Code ?** <Cevabınız>'
  ]
  let sayac = 0
  ...
  baschannel.send(`**Merhaba ${message.author}, Demek Sunucumuzda Yetkili Olmak İstiyorsun Ama Önce Bazı Soruları Cevaplaman Gerek Başarılar!\n:hourglass: Unutma!!! Tüm Soruları Cevaplaman İçin Tam 2 Dakikan Var Hızlı Ol :)**`);
  ...
  collector.on('end', collected => {
    ...
    baschannel.send('**Başvurunuz Başarıyla İletilmiştir!**\nBu Kanal 5 Saniye Sonra Silinecektir').then(
      setTimeout(function() {
        baschannel.delete();
         }, 5000));
    let sayac = 0
    ...
    .addField('Başvuran Hakkında',[
      ...
      `**Neden Anka Code: **\t\t${collected.map(m => m.content).slice(5,6)}`
    ])
    .setTimestamp()
    .setFooter('Developed by Gravity', message.guild.iconURL());
